...
export function CategoryNav({}: CategoryNavProps) {
	return (
		<div className="w-full">
			<div className="flex overflow-x-auto w-full py-4 px-4 sm:overflow-visible sm:py-0 sm:px-0 sm:grid sm:grid-cols-5 md:grid-cols-6 gap-4 sm:justify-items-center">
				{categories.map((category) => {
					...
					return (
						<div
							key={category.id}
							className="shrink-0 group flex flex-col items-center gap-2 cursor-pointer w-20 sm:w-28">
							<div
								className={cn(
								...
								{iconSrc ? (
									<img
										src={iconSrc}
										alt={category.name}
										className="h-10 w-10 sm:h-14 sm:w-14 opacity-80 group-hover:opacity-100 transition-opacity"
									/>
								) : (
